...
# This program has been modified from the original by team 5724 to allow for vertical
# tracking of the tape as well has horizontal.
...
#lower_green = np.array([59.89208633093526, 130.71043165467626, 18.345323741007192])
#upper_green = np.array([69.41979522184302, 255.0, 144.0358361774744])
...
def findBall(contours, image, centerX, centerY):
    ...
    if len(contours) > 0:
        ...
        if (len(biggestCargo) > 0):
            ...
            # These variable created by team 5724
            yCoord = closestCargo[1]
            finalTargetPitch = calculatePitch(yCoord, centerY, V_FOCAL_LENGTH)
            ...
            networkTable.putNumber("cargoPitch", finalTargetPitch)
...
def findTape(contours, image, centerX, centerY):
    ...
    if len(contours) >= 2:
        ...
        for i in range(len(biggestCnts) - 1):
            ...
            if (np.sign(tilt1) != np.sign(tilt2)):
                ...
                # This variable was added by team 5724
    ...
    if (len(targets) > 0):
        ...
        #This variable created by team 5724
        currentVerticalError = finalTarget[2]

        # pushes vision target angle to network tables
        networkTable.putNumber("tapeYaw", currentAngleError)

        networkTable.putNumber("tapePitch", currentVerticalError)
    else:
...
def calculateYaw(pixelX, centerX, hFocalLength):
    ...
    #return round(yaw)
    # Team 5724 changed this to disable rounding
    return yaw
